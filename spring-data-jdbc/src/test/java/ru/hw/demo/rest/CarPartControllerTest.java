...
class CarPartControllerTest {
    ...
    void shouldReturnCarPartByFilterInRequestParam() throws Exception {
        ...
        Brand brandOne = brandRepositoryDataJdbc.findById(cpOne.getBrandRef().getId()).get();
        Model modelOne = modelRepositoryDataJdbc.findById(cpOne.getModelRef().getId()).get();
        ...
        mvc.perform(
                        get("/api/v1/carparts")
                                .param("brandName", brandOne.getName()) // Gaz
                                .param("yearRelease", modelOne.getYearRelease().toString()) // 1964
                )
                .andExpect(status().isOk())
                .andExpect(content().json(mapper.writeValueAsString(expectedResult)));
    }
